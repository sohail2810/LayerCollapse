...
from GatedGPT2 import GPT2BlockGP
...
def train_with_trainer(model, train_dataloader, validation_dataloader, epochs=3, learning_rate=5e-3, train_batch_size=64, eval_batch_size=256, gp_weight=0, task = "sst2", lr_decay=0.9, weight_decay=0.01, use_sgd=False, grad_accumulation=1):
    if use_sgd:
...
def get_GP_model_gpt2(model, num_GP_layers, only_list = []):
    config = model.config
    copy_model = copy.deepcopy(model)
    if num_GP_layers <= 0:
        return copy_model
    if len(only_list) == 0:
        for name, module in list(copy_model.named_modules())[::-1]:
            if isinstance(module, GPT2Block):
                print("Setting up gated layer {}".format(name))
                new_module = GPT2BlockGP(config=config)
                new_module.load_from_basic(module=module)
                change_module(copy_model, name, new_module)
                num_GP_layers -= 1
                if num_GP_layers == 0:
        ...
        for name, module in list(copy_model.named_modules())[::-1]:
            if isinstance(module, GPT2Block) and name.split(".")[-1] in only_list:
                print("Setting up gated layer {}".format(name))
                new_module = GPT2BlockGP(config=config)
                new_module.load_from_basic(module=module)
                change_module(copy_model, name, new_module)
...
def collapse_GP_model(model, num_layers = 100):
    for name, module in list(model.named_modules())[::-1]:
        if (isinstance(module, GPT2BlockGP)) and num_layers > 0:
            print("Collapsing layer {}".format(name))
            module.purge_layer()
            num_layers -= 1